...
type ConnectInterceptor struct {
	// PreLoginResponse is used to obtain PreLogin Response fields
	PreLoginResponse chan map[uint8][]byte
	// ClientLogin is used to pass the client Login
	ClientLogin chan Login
	// ServerLoginAck is used to pass the server LoginAck
	ServerLoginAck chan LoginAckStruct
}
