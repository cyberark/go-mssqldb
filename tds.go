...
func writePreloginResponse(w *tdsBuffer, fields map[uint8][]byte) error {
	var err error
...
func PerformFakeHandshakeForClient(clientConn net.Conn) error {
	...
	clientBuf := newTdsBuffer(4096, clientConn)
	...
	msg[preloginTHREADID] = []byte{}
	msg[preloginVERSION] = []byte{0x0e, 0x00, 0x0c, 0xa6, 0x00, 0x00}
	msg[preloginENCRYPTION] = []byte{encryptNotSup}
